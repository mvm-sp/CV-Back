...
const params = require('../parameters/cvhistory');
...
async function getResume(req, res, next) {
  ...
  try {
    ...
    const queryResult = await client.query(querys.Resume ,params.buildResume(req).params);
    const { rows } = queryResult
    
    return res.json({ payload: rows[0].prgetjsoncvcontent });
